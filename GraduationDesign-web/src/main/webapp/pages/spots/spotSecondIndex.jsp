...
<body>
	<div
		style="width: 1146px; height: 30px; line-height: 30px; margin: 0 auto;">
		<p style="float: right; font-size: 14px; color: #666">全部分类
			》${productCategory.cateGoryName}</p>
	</div>
	<div style="width: 1146px; height: 185px; margin: 0 auto;">
		...
		<c:forEach items="${hotProduct}" var="h" begin="0" end="2">
			<div class="div_tmtop">
				<div class="div_temai">
					<img src="<%=path%>/images/spots/imageslist/tj_03.png" />
				</div>
				<div class="div_shangping">
					<div style="width: 154px; float: left;">
						<img style="width: 154px; height: 139px;"
							src="<%=path%>${h.filePath}" />
					</div>
					<div style="float: left; margin-left: 10px; width: 170px;">
						<p
							style="font-size: 15px; font-weight: 600; color: #5e5e5e; margin-bottom: 7px;">${h.productName}</p>
						<textarea class="questionBank" name="special.name"
							id="specialName" disabled="disabled">
${h.productTitle}
                     </textarea>
						<p id="p_jiage">
						...
						<input class="button_qg" type="button" /> <input
							class="button_sc" type="button" />
					</div>
				</div>
			</div>
		</c:forEach>
	...
	<div id="div_shaixuan">
		<p
			style="height: 38px; line-height: 38px; background-color: #FAFAFA; padding-left: 15px; border: 0;">
			<span style="font-size: 18px; color: #FE5C4D">${productCategory.cateGoryName}</span>
			<span style="font-size: 14px; color: #494949;">商品筛选</span> <span>（共125个商品）</span>
		</p>
		<p style="border: 0;">
		...
		<p>
			...
			<!----------------这里循环选择条件---------------------->
			<span onclick="price(0);">全部</span> <span onclick="price(1);">50以内</span>
			<span onclick="price(2);">50~100</span> <span onclick="price(3);">100~150</span>
			<span onclick="price(4);">150~200</span> <span onclick="price(5);">200以上</span>
		</p>
		<p>
	...
	<div style="width: 1128px; margin: 0 auto;">
		<p id="p_tiaojian">
			<span>配送地区：<input type="text" /></span> <span>排序： <input
				id="input_1" onclick="paixu(1);" type="button" value="销量" /> <input
				id="input_2" onclick="paixu(2);" type="button" value="价格" /> <input
				id="input_3" onclick="paixu(3);" type="button" value="上架时间" />
			</span> <span><input type="radio" />仅显示有货商品</span>
		</p>
		<!-----------------------------左边------------------------------------------->
		<div style="width: 225px; float: left; border: 1px solid #e5e5e5;">
			<p
				style="height: 38px; line-height: 38px; border-top: 2px solid #FE5C4D; margin: 0; padding-left: 25px; color: #787878;">最近浏览</p>
			<c:forEach items="${recentProducts}" var="r" begin="0" end="5">
				<!-----------------------------这里开始循环------------------------------------------->
				<div class="div_liulan">
					<div
						style="width: 167px; height: 149px; padding: 1px; border: 1px solid #DFDFDF;">
						<img style="width: 167px; height: 149px;"
							src="<%=path%>${r.filePath}" />
					</div>
					<p style="font-size: 16px; color: #FE5C4D">￥${r.spotUnitPrice}</p>
					<textarea class="questionBank" name="special.name" id="specialName"
						disabled="disabled">
${r.productTitle}
                </textarea>
				</div>
		...
		<!-----------------------------右边------------------------------------------->
		<div style="width: 895px; float: right;"></div>
		<!-----------------------------这里开始循环------------------------------------------->
...
${p.productTitle}
				...
				<input class="button_qg" type="button" /> <input class="button_sc"
					type="button" />
			</div>
			<!-----------------------------这里开始循环------------------------------------------->
		</c:forEach>
