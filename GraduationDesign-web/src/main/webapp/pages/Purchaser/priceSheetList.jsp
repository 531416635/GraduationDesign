	...
	<div class="rightinfo">
		...
		<table class="tablelist">
			...
			<tbody>
				...
				<c:forEach items="${priceSheet}" var="c">
					<tr>
						<td style="display: none">${c.id}</td>
						<td style="display: none">${c.userId}</td>
						<td>${c.priceSheetId}</td>
						<td>${c.quantity}</td>
						<td>${c.productUnit}</td>
						<td>${c.unitPrice}</td>
			...
			<ul class="paginList">
				<c:choose>
					...
					<c:otherwise>
						<li class="paginItem"><a
							href="/GraduationDesign/main/priceSheetList.do?currentPage=${page.currentPage-1}&&userId=${userId}"><span
								class="pagepre"></span> </a></li>
					</c:otherwise>
				</c:choose>
				<c:forEach begin="1" end="${page.totalPage}" var="s3">
					<c:choose>
						...
						<c:otherwise>
							<li class="paginItem"><a
								href="/GraduationDesign/main/priceSheetList.do?currentPage=${s3}&&userId=${userId}">${s3}</a>
							</li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
