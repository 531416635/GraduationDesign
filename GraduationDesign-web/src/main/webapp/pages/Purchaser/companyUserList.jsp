 ...
 function getText(){
            ...
            xhr.onreadystatechange=function(){
                if(xhr.readyState==4 && xhr.status==200){
                    window.location.href="toCompanyUserList.do";
                }
            };
           xhr.open("post","deleteCompanyUsers1.do",true);
...
<body >
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li><a class="tablelink" href="/ssm/users/toCompanyAddUser.do"><span><img src="../images/t01.png" /></span>添加</a></li>
    ...
    <table class="tablelist">
        ...
        <c:forEach items="${userManagers}" var="userManager">
         <c:if test="${userManager.status==0 or userManager.status==1 or userManager.status==2}"> 
        <tr>
        <td><input type="checkbox" name="box" onclick="checkBox();" value="${userManager.id}" /></td>
         <td>${userManager.id}</td>
		        ...
		        <td><a href="toUserManagerAll1.do?id=${userManager.id }"  class="tablelink">详细信息</a> <a href="toUserManagerUpdate1.do?id=${userManager.id }" class="tablelink">修改</a>  <a href="javascript:checkDelete(${userManager.id});" class="tablelink"> 删除  </a>
		        </td>
		            </tr> 
		             </c:if> 
        </c:forEach>
 ...
 <div class="pagin">
        ...
        <ul class="paginList">
        		<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/ssm/products/productsList.do?currentPage=${page.currentPage-1}"><span class="pagepre"></span></a></li>
                		</c:otherwise>
                </c:choose>
        		<c:forEach begin="1" end="${page.totalPage}" var="s3">
					 	<c:choose>
                    		...
                    		<c:otherwise>
                    		<li class="paginItem"><a href="/ssm/products/productsList.do?currentPage=${s3}">${s3}</a></li>
                    		</c:otherwise>
                    	</c:choose>
				</c:forEach>
				<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/ssm/products/productsList.do?currentPage=${page.currentPage+1}"><span class="pagenxt"></span></a></li>
                		</c:otherwise>
                </c:choose>    
        </ul>
    </div>
