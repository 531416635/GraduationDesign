            ...
            xhr.onreadystatechange=function(){
                if(xhr.readyState==4 && xhr.status==200){
                    if (window.confirm("确认吗?")) {
						window.location.href="payable.do";
						} else {
						return;
						}
                }
            };
           xhr.open("post","allDeletePay.do",true);
...
$(function(){
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="payable.do?currentPage="+(page_index+1);
		}
		flag=true;
		return false;	
	   }
...
<body>
    ...
    <table class="tablelist">
        ...
        <c:forEach items="${pays }" var="pay">
	        <tr>
		        <td><input name="box" type="checkbox" value="${pay.id}" /></td>
		        <td>${pay.id}</td>
		        <td>${pay.settlementId}</td>
		        <td>${pay.currency}</td>
		        <td>${pay.settleAmount}</td>
		        <td>${pay.settleFee}</td>
		        <td>${pay.settleBy}</td>
		        <td>${pay.settleTime}</td>
		        <td>${pay.status}</td>
		        <td>${pay.bankAccount}</td>
		        <td>${pay.settleType}</td>
		        <td>
		           <c:choose>
                   		<c:when test="${pay.isInvoice==0}">
                   			否
                   		</c:when>
                   		<c:when test="${pay.isInvoice==1}">
                   			是
                   		</c:when>
                   </c:choose>
		        </td>
		        <td><a href="toPayableUpdate.do?id=${pay.id }" class="tablelink">修改</a>  <a href="payableDelete.do?id=${pay.id }" class="tablelink"> 删除  </a></td>
	        </tr> 
        </c:forEach>
         </tbody>
    </table>
...
<%--     <div class="pagin">
        ...
        <ul class="paginList">
        		<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/ssm/supplys/payable.do?currentPage=${page.currentPage-1}"><span class="pagepre"></span></a></li>
                		</c:otherwise>
                </c:choose>
        		<c:forEach begin="1" end="${page.totalPage}" var="s3">
					 	<c:choose>
                    		...
                    		<c:otherwise>
                    		<li class="paginItem"><a href="/ssm/supplys/payable.do?currentPage=${s3}">${s3}</a></li>
                    		</c:otherwise>
                    	</c:choose>
				</c:forEach>
				<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/ssm/supplys/payable.do?currentPage=${page.currentPage+1}"><span class="pagenxt"></span></a></li>
                		</c:otherwise>
                </c:choose>    
        </ul>
    </div> --%>
   ...
   <div class="tip">
      ...
      <div class="tipinfo">
        ...
        <div class="tipbtn">
        <input name="" type="button"  class="sure" value="确定" onclick="location='toPayableAdd.do'"/>&nbsp;
