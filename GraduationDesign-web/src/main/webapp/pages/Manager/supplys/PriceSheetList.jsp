...
<script type="text/javascript">
	...
	function getText() {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				if (window.confirm("确定要删除吗?")) {
					window.location.href = "PriceSheet.do";
				} else {
					return;
				}
		...
		xhr.open("post", "allDeletePriceSheet.do", true);
...
$(function(){
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="PriceSheet.do?currentPage="+(page_index+1);
		}
		flag=true;
		return false;	
	   }
		...
		<tbody>
			<c:forEach items="${dispatch}" var="dis">
				<tr>
					...
					<td>${dis.priceSheetId}</td>
					<td>${dis.companyProductId}</td>
					<td></td>
					<td>${dis.quantity}</td>
					<td>${dis.productUnit}</td>
					<td><c:if test="${dis.unitPrice!=null}">${dis.unitPrice}</c:if></td>
					...
					<td><a href="deletePriceSheet.do?id=${dis.id }"
						class="tablelink"> 删除 </a> <a
						href="toPriceSheetListUpdate.do?id=${dis.id}" class="tablelink">修改</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
