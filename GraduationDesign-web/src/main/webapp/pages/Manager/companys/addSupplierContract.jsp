		...
		<table border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<td><input id="companyId" name="companyId" type="hidden"
					required="required" value="${companyId}" /></td>
			</tr>

			<tr>
				<th>合同账期:</th>
				<td><input name="contractTerms" id="contractTerms"
					value="${c.contractTerms}" required="required" /></td>
			</tr>
			<tr>
				<th>合同时间:</th>
				<td><input name="contractTime" id="contractTime"
					required="required" onclick="WdatePicker()" readonly="readonly"
					value="${contractTime}" /></td>
			</tr>
			<tr>
				<th>合同期限:</th>
				<td><input name="contractExpirationTime"
					id="contractExpirationTime" required="required"
					onclick="WdatePicker()" readonly="readonly"
					value="${contractExpirationTime}" /></td>
			</tr>
			<%-- <tr style="display: none"> 
				<th>供应品类:</th>
			...
			<tr>
				...
				<td><input name="contractType" id="contractType"
					required="required" value="供应商" readonly="readonly" /></td>
			</tr>
			<tr>
				<th>状态:</th>
				<td><input type="radio" id="status" name="status" value="0"
					required="required"><span>签订</span> <input type="radio"
					id="status" name="status" value="1" required="required"><span>未签订</span>
				</td>
			</tr>
			<tr>
				<th>合同内容:</th>
				<td><textarea rows="15" cols="30" id="contractContent"
						name="contractContent" value="${contractContent}"
						required="required"></textarea></td>
			</tr>
		</table>
		<input type="button" onclick="addBuyerContract()" value="增加" /> <input
			type="reset" value="取消" onclick="history.back()">
		<!-- 	</form> -->
	</div>
</body>
</html>
height: 40px; line-height: 40px; } .info th { text-align: right; width:
65px; color: #4f4f4f; padding-right: 5px; font-size: 13px; } .info td {
text-align: left; }
</style>
</head>
<body>
	<div>
		<form action="addSupplierContract.do?contractType=${contractType }"
			method="post">
			<table border="0" cellpadding="0" cellspacing="0">
				<tr style="display: none">
					<th>公司ID</th>
				...
				<tr>
					<th>合同账期:</th>
					<td><input name="contractTerms" value="${contractTerms}"
				...
				<tr>
					<th>状态:</th>
					<td><input type="radio" name="status" value="0"><span>签订</span>
				...
				<tr>
					<th>合同类型:</th>
					<td><input name="contractType" id="contractType"
						required="required" value="采购商" readonly="readonly" /></td>
				</tr>
				<tr>
					<th>合同内容:</th>
					<td><input name="contractContent" value="${contractContent}"
						required="required" /></td>
				</tr>
			</table>
