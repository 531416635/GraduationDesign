...
<body>
	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr class="info">
			<th>类别名称:</th>
			<td><input type="text" name="typeName" value="${productType.typeName}"/></td>
		</tr>
		<tr class="info">
			<th>父类别:</th>
			 <td>
			...
			<option value="${pr.id}"
			<c:if test="${productType.parentTypee.id==pr.id}"> selected</c:if>
			>${pr.typeName}</option>
			</c:forEach>
			</select>
			</td> 
		...
		<tr class="info">
			<th>类别简称:</th>
			<td><input type="text" name="typeCode" value="${productType.typeCode}"/></td>
		</tr>
	</table>
