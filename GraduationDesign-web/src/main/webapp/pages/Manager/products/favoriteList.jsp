...
function checkDelete(id){
	 ...
	 if(r){
             location.href="favoriteDelete.do?id="+id;
          }
}
...
<body >
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li><a class="tablelink" href="/ssm/favorite/tofavoriteAdd.do"><span><img src="../images/t01.png" /></span>添加</a></li>
    ...
    <table class="tablelist">
        ...
        <tr>
        <td><input type="checkbox" name="box" onclick="checkBox();" value="${f.id}"/></td>
        <td>${f.id}</td>
        <td>${f.product.productName}</td>
        <td>${f.favoriteName}</td>
        <td>
        <c:choose>
        <c:when test="${f.favoriteType==1}">
       	 商品
        </c:when>
        <c:when test="${f.favoriteType==2}">
       	模版
        </c:when>
        </c:choose>
        </td>
        <td>
        <a class="tablelink" href="/ssm/favorite/favoriteProductInfo.do?id=${f.product.id}">商品详情</a>
        <a class="tablelink" href="/ssm/favorite/tofovoriteUpdate.do?id=${f.id}">修改</a>
       	<a class="tablelink" href="#" onclick="checkDelete(${f.id});">删除</a>
        </td>
        </tr> 
        </c:forEach>
        </tbody>
 ...
 <div class="pagin">
        ...
        <ul class="paginList">
        		<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/ssm/favorite/favoriteList.do?currentPage=${page.currentPage-1}"><span class="pagepre"></span></a></li>
                		</c:otherwise>
                </c:choose>
        		<c:forEach begin="1" end="${page.totalPage}" var="s3">
					 	<c:choose>
                    		...
                    		<c:otherwise>
                    		<li class="paginItem"><a href="/ssm/favorite/favoriteList.do?currentPage=${s3}">${s3}</a></li>
                    		</c:otherwise>
                    	</c:choose>
				</c:forEach>
				<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/ssm/favorite/favoriteList.do?currentPage=${page.currentPage+1}"><span class="pagenxt"></span></a></li>
                		</c:otherwise>
                </c:choose>    
        </ul>
    </div>
