			...
			<tbody>
				...
				<c:forEach items="${companyProducts}" var="c">
					<tr>
						...
						<td>
							<%-- ${c.product.productTypes} --%> <c:forEach
								items="${c.product.productTypes}" var="m" varStatus="s">
								<c:choose>
									<c:when test="${s.last}">
                            				${m.typeName}
                            			</c:when>
						...
						<td><a class="tablelink"
							href="/GraduationDesign/supplyCompanyProducts/toCompanyProductPriceInfo.do?id=${c.id}">价格详情</a>
							<a class="tablelink"
							href="/GraduationDesign/supplyCompanyProducts/toUpdateCompanyProducts.do?id=${c.id}">修改</a>
