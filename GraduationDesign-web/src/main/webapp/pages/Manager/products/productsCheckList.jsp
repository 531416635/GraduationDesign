...
<body >
    ...
    <table class="tablelist">
        ...
        <td>${c.productName}</td>
          <c:choose>
        	<c:when test="${c.status==0 }"><td>未审核</td></c:when>
        	<c:when test="${c.status==1 }"><td>已通过</td></c:when>
        	<c:when test="${c.status==2 }"><td>未通过</td></c:when>
        	<c:when test="${c.status==3 }"><td>已下架</td></c:when>
        	<c:otherwise><td></td></c:otherwise>
        </c:choose>
        <td>
        <a class="tablelink" href="/ssm/products/approveCheck.do?id=${c.id}">批准通过</a>
       <a class="tablelink" href="/ssm/products/disapproveCheck.do?id=${c.id}">未通过</a>
       <a class="tablelink" href="/ssm/products/deleteCheck.do?id=${c.id}">下架</a>
        </td>
        </tr> 
        </c:forEach>
        </tbody>
