...
$(function(){
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="companyProductsList.do?currentPage="+(page_index+1);
		}
		flag=true;
		return false;	
	   }
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li><a class="tablelink" href="/ssm/companyProducts/toAddCompanyProducts.do"><span><img src="../images/t01.png" /></span>添加</a></li>
    ...
    <table class="tablelist">
        ...
        <td>${c.id}</td>
        <td><a class="tablelink" href="/ssm/companyProducts/toCompanyInfo.do?id=${c.company.id}">${c.company.companyName}</a></td>
        <td><%-- <a class="tablelink" href="/ssm/companyProducts/toProductInfos.do?id=${c.product.id}">${c.product.productName}</a> --%>
          <a class="tablelink" href="/ssm/companyProducts/toCompanyProductPicInfo.do?id=${c.id}&productId=${c.product.id}">${c.product.productName}</a>
        </td>
        <c:choose>
        	<c:when test="${c.status==0 }"><td>可用</td></c:when>
        	<c:when test="${c.status==1 }"><td>冻结</td></c:when>
        	<c:when test="${c.status==2 }"><td>删除</td></c:when>
        	<c:otherwise><td></td></c:otherwise>
        </c:choose>
        <td>
      
        <a class="tablelink" href="/ssm/companyProducts/toCompanyProductPriceInfo.do?id=${c.id}">价格详情</a>
        <a class="tablelink" href="/ssm/companyProducts/toProductTransfer.do?id=${c.id}">物流追朔</a>
        <a class="tablelink" href="/ssm/companyProducts/toUpdateCompanyProducts.do?id=${c.id}">修改</a>
