 ...
 function getText(){
            ...
            xhr.onreadystatechange=function(){
                if(xhr.readyState==4 && xhr.status==200){
                     if (window.confirm("确认吗?")) {
						window.location.href="rights.do";
						} else {
						return;
						}
                }
            };
           xhr.open("post","allDeleteR.do",true);
  ...
  $(function(){
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="rights.do?currentPage="+(page_index+1);
		}
		flag=true;
		return false;	
	   }
	...
	<div class="rightinfo">
        ...
        <ul class="paginList">
        		<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/GraduationDesign/users/rights.do?currentPage=${page.currentPage-1}"><span class="pagepre"></span></a></li>
                		</c:otherwise>
                </c:choose>
        		<c:forEach begin="1" end="${page.totalPage}" var="s3">
					 	<c:choose>
                    		...
                    		<c:otherwise>
                    		<li class="paginItem"><a href="/GraduationDesign/users/rights.do?currentPage=${s3}">${s3}</a></li>
                    		</c:otherwise>
                    	</c:choose>
				</c:forEach>
				<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="/GraduationDesign/users/rights.do?currentPage=${page.currentPage+1}"><span class="pagenxt"></span></a></li>
                		</c:otherwise>
                </c:choose>    
        </ul>
    </div> --%>
