...
$(function(){
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="getAllWorkflowLogs.do?currentPage="+(page_index+1);
		}
		flag=true;
		return false;	
	   }
	...
	<%--  <div class="pagin">
        ...
        <ul class="paginList">
        		<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="getAllWorkflowLogs.do?currentPage=${page.currentPage-1}"><span class="pagepre"></span></a>
                		</c:otherwise>
                </c:choose>
        		<c:forEach begin="1" end="${page.totalPage}" var="s3">
					 	<c:choose>
                    		...
                    		<c:otherwise>
                    		<li class="paginItem"><a href="getAllWorkflowLogs.do?currentPage=${s3}">${s3}</a>
                    		</c:otherwise>
                    	</c:choose>
				</c:forEach>
				<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="getAllWorkflowLogs.do?currentPage=${page.currentPage+1}"><span class="pagenxt"></span></a>
                		</c:otherwise>
                </c:choose>    
        </ul>
    </div>
