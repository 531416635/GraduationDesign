...
<script type="text/javascript">
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				if (window.confirm("确定要删除吗?")) {
					window.location.href = "getAllWorkFlowNodes.do";
				} else {
					return;
				}
			}
		};
		xhr.open("post", "allDeleteWorkFlowNodes.do", true);
	...
	$(function() {
		...
		function pageselectCallback(page_index, jq) { //此函数为分页动态按钮展现的回调函数，即分页展现商品
			if (flag) {
				location.href = "getAllWorkFlowNodes.do?currentPage="
						+ (page_index + 1);
			}
			flag = true;
			return false;
		...
		<tbody>
			<c:forEach items="${dispatch}" var="dis">
				<tr>
					...
					<td><a href="deleteWorkFlowNodes.do?id=${dis.id }"
						class="tablelink"> 删除 </a> <a
						href="toWorkFlowNodesUpdate.do?id=${dis.id}" class="tablelink">修改</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<%--  <div class="pagin">
        ...
        <ul class="paginList">
        		<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="getAllWorkFlowNodes.do?currentPage=${page.currentPage-1}"><span class="pagepre"></span></a>
                		</c:otherwise>
                </c:choose>
        		<c:forEach begin="1" end="${page.totalPage}" var="s3">
					 	<c:choose>
                    		...
                    		<c:otherwise>
                    		<li class="paginItem"><a href="getAllWorkFlowNodes.do?currentPage=${s3}">${s3}</a>
                    		</c:otherwise>
                    	</c:choose>
				</c:forEach>
				<c:choose>
                		...
                		<c:otherwise>
                		 <li class="paginItem"><a href="getAllWorkFlowNodes.do?currentPage=${page.currentPage+1}"><span class="pagenxt"></span></a>
                		</c:otherwise>
                </c:choose>    
        </ul>
    </div> --%>
