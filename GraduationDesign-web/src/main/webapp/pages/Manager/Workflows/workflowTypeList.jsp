...
<script type="text/javascript">
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				if (window.confirm("确定要删除吗?")) {
					window.location.href = "getAllWorkflowType.do";
				} else {
					return;
				}
			}
		};
		xhr.open("post", "allDeleteWorkflowType.do", true);
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="getAllWorkflowType.do?currentPage="+(page_index+1);
		}
		flag=true;
		return false;	
	   }
...
<body>
	...
	<div class="tools">
		<ul class="toolbar">
			<li><a class="tablelink" href="toWorkflowTypeAdd.do?"><span><img
		...
		<tbody>
			<c:forEach items="${dispatch}" var="dis">
				<tr>
					...
					<td>${dis.id}</td>
					<td>${dis.workflowTypeName}</td>
					<td>${dis.createBy}</td>
					<td>${dis.createTime}</td>
					<td>${dis.status}</td>

					<td><a href="deleteWorkflowType.do?id=${dis.id }"
						class="tablelink"> 删除 </a> <a
						href="toWorkflowTypeUpdate.do?id=${dis.id}" class="tablelink">修改</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
		...
		<ul class="paginList">
			<c:choose>
				...
				<c:otherwise>
					<li class="paginItem"><a
						href="getAllWorkflowType.do?currentPage=${page.currentPage-1}"><span
							class="pagepre"></span> </a>
				</c:otherwise>
			</c:choose>
			<c:forEach begin="1" end="${page.totalPage}" var="s3">
				<c:choose>
					...
					<c:otherwise>
						<li class="paginItem"><a
							href="getAllWorkflowType.do?currentPage=${s3}">${s3}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:choose>
