...
<script type="text/javascript">
	...
	if(id.length>0 &&addrsss.length>0){
	location.href="addMyCollection.do?favoriteType=2&&pid="+id+"&&addrsss="+addrsss;
	}else{
	alert("请选择正确的商品");
	}
	}
	...
	//抢购，生成对应订单
	var qianggou = function() {
	
	var ids = "";
	var productQuantity="";
	var addressShopping="";
		$("input[name=box" + 1 + "]:checked").each(function() {
			ids += $(this).val() + ",";
			productQuantity+=$("#ProductQuantity" + $(this).val()).val()+ ",";
		 	if(addressShopping==""){ 
				addressShopping=$("input[name=addressShopping" + $(this).val() + "]").val();
			$("#ProductQuantity" + $(this).val()).val();
			 	alert(addressShopping);
			 } else if(addressShopping!=$("input[name=addressShopping" + $(this).val() + "]").val()){
			 	alert("请选择相同区域的送货地址");
			 	return false;
			 }
		});
		ids =ids.substring(0, ids.length - 1);
		productQuantity=productQuantity.substring(0, ids.length - 1);
		alert(ids);
 ...
 $(function(){
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		...
		location.href="spotShoppingCar.do?currentPage="+(page_index+1)+"&&userId="+<%=userId%>;
		}
			...
			<table width="100%" cellspacing="0" align="center">
				<thead>
					<tr id="t">
						...
						<th><input name="" id="checkAll1" type="checkbox"
					...
					<c:forEach items="${list}" var="c">
						...
						<tr>
							...
							<td><input type="checkbox" name="box1"
								value="${c.ShoppingCartId}" onclick="checkBox(1);" /> <%-- <input type="radio" name="box" id="shoppingCartId"
