...
<script type="text/javascript">
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="/GraduationDesign/merchant/myCollection.do?favoriteType=2&&currentPage1="+(page_index+1)+"&&show=1";
		}
		flag=true;
		return false;	
	   }
	...
	function pageselectCallback(page_index, jq){  //此函数为分页动态按钮展现的回调函数，即分页展现商品
		if(flag){
		location.href="/GraduationDesign/merchant/myCollection.do?favoriteType=2&&currentPage2="+(page_index+1)+"&&show=2";
		}
		flag=true;
		return false;	
	   }
		...
		<div class="detail_list">
			<ul id="tabs">
				<li><a
					...
					title="tab1"
					style="background: #9a1101 none repeat scroll 0 0; color: #fff;">现货收藏</a></li>
				<li><a
					href="<%=path %>/merchant/myCollection.do?favoriteType=1"
					title="tab2">商品收藏</a></li>
			</ul>
			<div id="content">
				<div id="tab1">
						...
						<tbody>
							<c:forEach items="${list}" var="c">
								<tr>
									...
									<td>${c.ProductName}</td>
									<td><input style="width: 80px; height: 20px;"
										id="buyNum${c.ProductID}" name="buyNum${c.ProductID}"
									...
									<td><c:if test="${c.ProductType==1  }">
										...
										</c:if> <c:if test="${c.ProductType!=1  }">我要询价</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
