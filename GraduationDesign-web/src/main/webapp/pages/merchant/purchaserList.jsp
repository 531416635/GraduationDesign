...
<body style="font-family:"MicrosoftYaHei",Arial,Helvetica,sans-serif,"宋体";">
	...
	<div class="inner_div">
		...
		%>
			<p id="p1">
				<span>议价管理：有</span><span class="span_p"><a
					style="color: #B00100; font-size: 22px;" href="myPriceSheet.do">${rows1}</a>
				</span><span>个议价单供应商已响应</span>
			</p>
			<%
			}
		%>
		...
		%>
			<p id="p1">
				<span>配送管理：有</span><span class="span_p"><a
					style="color: #B00100; font-size: 22px;" href="myOrder.do">${rows2}</a></span><span>个订单等待您分配地址</span>
			</p>
			<%
			}
		%>
		...
		%>
			<p id="p1">
				<span>验收管理：有</span><span class="span_p"><a
					style="color: #B00100; font-size: 22px;"
					href="myDispatchBillToChecker.do">${rows3}</a></span><span>个验收单等待您的处理</span>
			</p>
			<%
			}
		%>
		...
		%>
			<p id="p1">
				<span>议价管理：有</span><span class="span_p"><a
					style="color: #B00100; font-size: 22px;" href="Bargaining.do">${r1}</a></span><span>个议价单等待您的处理</span>
			</p>
			<%
			}
		%>
		...
		%>
			<p id="p1">
				<span>配送管理：有</span><span class="span_p"><a
					style="color: #B00100; font-size: 22px;"
					href="myDispatchBillToSupplier.do">${r2}</a></span><span>个订单等待您发货配送</span>
			</p>
			<%
			}
		%>
		...
		%>
			<p id="p1">
				<span>验收管理：有</span><span class="span_p"><a
					style="color: #B00100; font-size: 22px;"
					href="myDispatchBillToDeliveryUser.do">${r3}</a></span><span>个验收单等待您的处理</span>
			</p>
			<%
			}
		%>
		...
		%>
		<div id="d1">
			对账单信息 <a href="financeInfo.do"><input class="orange" id="b1"
				type="button" value="我的财务" /> </a>
			<hr />
			<div class="div_list">
		...
		%>
		<div id="d2">
			购物车&nbsp;&nbsp;&nbsp;&nbsp; <a href="#"><input class="orange"
				id="b2" type="button" value="我的购物车" /> </a>
			<hr />
			<div class="div_list">
				<table width="200" border="1" cellspacing="0">
					<thead>
		...
		<div id="d3" style="height: 236px;">
			<p id="p_head">
				我的商品收藏 <span
					style="float: right; margin-right: 20px; display: block;"><a
					href="<%=path%>/merchant/myCollection.do?favoriteType=1">更多>></a></span>
			</p>
			<div style="margin-left: 10px;">
				<c:forEach items="${favorites}" var="f" begin="0" end="5">
					<div class="div_Commodity">
						...
						<div class="div_anniu">
							<input id="anniu" class="orange" type="button" value="询价"
								onclick="xunjia(${f.ID},${f.ProductID});" />
						</div>
					</div>
				</c:forEach>
			</div>
		...
		<div id="d4" style="height: 236px;">
			<p id="p_head">
				我的商品供应 <span
					style="float: right; margin-right: 20px; display: block;"><a
					href="<%=path%>/merchant/toProductEdit3.do">更多>></a></span>
			</p>
			<div style="margin-left: 10px;">
				<c:forEach items="${companyProducts}" var="cp" begin="0" end="5">
					<div class="div_Commodity">
						...
						<div class="div_anniu">
							<c:if test="${cp.Status == 0}">
								<input class="anniu_sxjia" type="button" value="上架"
									onclick="sjia(${cp.ID});" />
							</c:if>
							<c:if test="${cp.Status == 1}">
								<input class="anniu_sxjia" type="button" value="下架"
									onclick="xjia(${cp.ID});" />
							</c:if>
						</div>
					</div>
		...
		<div id="d5" style="height: 192px;">
			...
			<div
				style="float: left; height: 147px; width: 374px; border-right: 1px solid #D1D1D1; padding: 10px 0 0 25px;">
				...
				<p id="p4" style="height: 34px;">
					...
					<span style="margin-top: 10px;"><input
						style="color: #787878; font-weight: 700;" type="button" value="充值" /></span>
					<span style="margin-top: 10px;"><input
						style="color: #787878; font-weight: 700;" type="button" value="提现" /></span>
				</p>
			</div>
			<c:if test="${companyType==1 || companyType==2}">
				<div
					style="float: left; height: 157px; width: 235px; text-align: center;">
					<P style="font-size: 14px; color: #484442;">累计采购金额：</p>
					<P style="height: 30px; padding-left: 60px;">
						<span style="font-size: 20px; color: #787878;">${CountPurchaseAmount}</span><span
							style="font-size: 14px; color: #484442; margin-top: 8px;">元</span>
					</p>
					<P>
						<input style="color: #787878; font-weight: 700;" type="button"
							value="查看" onclick="showLog();" />
					</p>
			...
			<c:if test="${companyType==1 || companyType==3}">
				<div
					style="float: left; height: 157px; width: 234px; text-align: center; border-left: 1px solid #D1D1D1;">
					<P style="font-size: 14px; color: #484442;">累计供应金额：</p>
					<P style="height: 30px; padding-left: 60px;">
						<span style="font-size: 20px; color: #787878;">${CountSupplyAmount}</span><span
							style="font-size: 14px; color: #484442; margin-top: 8px;">元</span>
					</p>
					<P>
						<input style="color: #787878; font-weight: 700;" type="button"
