...
<body>
	<div class="top">
         <form id="form" action="<%=path%>/weChat/findMyInquiry.do" method="post">
         	<P><span>询价单号：</span><input type="text" name="inquiryId" id="inquiryId"/></P>
            <P><span>商品名称：</span><input type="text" name="productName" id="productName"/></P>
            <P><span>询价日期：</span><input style="width:35%" type="text" name="inquiryTimeStart" id="inquiryTimeStart" onclick="WdatePicker()" readonly="readonly"/>-<input style="width:35%" type="text" name="inquiryTimeStop" id="inquiryTimeStop" onclick="WdatePicker()" readonly="readonly"/></P>
         ...
         <table>
            ...
            <tr>
            	<td>日期：</td>
                <td>
                <fmt:formatDate value="${it.orderTime}"
								pattern="yyyy-MM-dd" />
                </td>
            </tr>
            <tr>
            	<td>状态：</td>
                <td>
               	<c:if test="${it.status==0}">未询价</c:if>
               	<c:if test="${it.status==1}">已询价</c:if>
                </td>
            </tr>
         </table>
         </c:forEach>
